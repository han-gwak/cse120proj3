...
public class Communicator {
  ...
  private Lock lock;
  private Condition2 CVSpeak;
  private Condition2 CVListen;
  private Condition2 CVTransfer;
  ...
  public Communicator() {
    this.lock = new Lock();
    this.CVSpeak = new Condition2(lock);
    this.CVListen = new Condition2(lock);
    this.CVTransfer = new Condition2(lock);
  }
  ...
  public void speak(int word) {
    ...
    // sleep while buffer full
    while(bufferFull) {
      CVSpeak.sleep();
    }

    // transfer word - wake CVTransfer in listen once word transfers
    transferWord = word;
    bufferFull = true;
    CVListen.wake();

    // speaker wakes after listener reads data
    CVTransfer.sleep();
    lock.release();
  }
  ...
  public int listen() {
    ...
    // sleep until word written by speak()
    while(!bufferFull) {
      CVListen.sleep();
    }

    // save data, let transfer and speak wake
    bufferFull = false;
    int word = transferWord;
    transferWord = 0;
    CVTransfer.wake();
    CVSpeak.wake();
    lock.release();
